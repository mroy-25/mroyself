...
alasan = '-'
//=================================================//
module.exports = sanz = async (sanz, mek) => {
	try {
		...
		const botNumberss = sanz.user.jid + '@c.us'
...
module.exports = sanz = async (sanz, mek) => {
	try {
		...
		const q = args.join(' ')
		const botNumber = sanz.user.jid
		const ownerNumber = [`6289656439589@s.whatsapp.net`, `6282221066105@s.whatsapp.net`] //sanz owner
		const botNumberss = sanz.user.jid + '@c.us'
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
...
❏ *SA-BOT Powered by self* ❏`
    ...
    case prefix+ 'revip':
           ...
           .catch((error) => {
			...
			break
	case prefix+'waifu':
	case prefix+'waifukawai':
			axios.get(`https://nekos.life/api/v2/img/kemonomimi`)
			.then((res)=>{
			imageToBase64(res.data.url)
			.then((response) => {var buff = Buffer.from(response, 'base64');
			sanz.sendMessage(from, buff, image, {quoted: mek, caption: 'Dih Wibu..'})
			})})
           .catch((err) => {
            reply('Error bang wkwkwk'); 
            })
			break
	case prefix+'husbu':
			husbud = await fetchJson(`https://api.fdci.se/rep.php?gambar=husbu`, {method: 'get'})
			...
			break
	case prefix+'hentai':
			anu = await fetchJson(`https://mnazria.herokuapp.com/api/nhentai?code=212121`, {method: 'get'})
	...
	case prefix+'pinterest':
			goblog = args.join(" ")
			anu = await fetchJson(`https://api.fdci.se/rep.php?gambar=${goblog}`, {method: 'get'})
			sasu = JSON.parse(JSON.stringify(anu));
			ke =  sasu[Math.floor(Math.random() * sasu.length)];
			nye = await getBuffer(ke)
			sanz.sendMessage(from, nye, image, { caption: 'Powered bai gatau pinterest lah bang awokwok', quoted: mek })
